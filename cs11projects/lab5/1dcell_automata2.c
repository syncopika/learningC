...
void setArray(char *array, int length){
	...
	for(i = 1; i < length-1; i++){
		...
		if(random > 5){
			*array = FULL; /* deref to access contents */
		}else{
			*array = EMPTY;
		}
...
void update(char *arr1, char *arr2, int length)
{
	...
	/* set up pointers */
	char *pointerToCell = arr1 + 1;
	char *pointerToLeft = arr1 + 0;
	char *pointerToRight = arr1 + 2;
	char *pointerToOtherArray = arr2 + 1;
	
	/* figure out what next generation will look like. put it in arr2 */
	for(i = 1; i < length-1; i++){
		
		if(changeCell(*(pointerToCell), *(pointerToLeft), *(pointerToRight)) == 1){
			*(pointerToOtherArray) = FULL;
		}else{
			*(pointerToOtherArray) = EMPTY;
		}
	...
	/* reset pointerToOtherArray and pointerToCell */
	pointerToCell = arr1;
	pointerToOtherArray = arr2;
	for(i = 0; i < length-1; i++){
		*(pointerToCell) = *(pointerToOtherArray);
		pointerToCell++;
		pointerToOtherArray++;
	}
}
